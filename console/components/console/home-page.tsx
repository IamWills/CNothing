...
export function HomePage() {
  ...
  return (
    <PageFrame
      ...
      actions={<><ReloadIconButton onReload={() => void refresh()} disabled={loading} /><ChannelRouteTabs items={homeChannelTabs} /></>}
    >
      <ConnectionPanel draft={draft} onDraftChange={setDraft} onApply={saveDraft} connection={connection} errorMessage={errorMessage} />
      ...
      <Card className="space-y-4">
        ...
        <ol className="grid gap-3 text-sm md:grid-cols-4">
          <li className="rounded-2xl bg-slate-50 p-4"><strong>1. Connect</strong><p className="mt-2 text-slate-600">The user connects a provider once.</p></li>
          <li className="rounded-2xl bg-slate-50 p-4"><strong>2. Request</strong><p className="mt-2 text-slate-600">The Agent requests a scoped grant.</p></li>
          <li className="rounded-2xl bg-slate-50 p-4"><strong>3. Approve</strong><p className="mt-2 text-slate-600">Approve in the browser or iOS app.</p></li>
          <li className="rounded-2xl bg-slate-50 p-4"><strong>4. Proxy</strong><p className="mt-2 text-slate-600">CNothing injects credentials server-side.</p></li>
        </ol>
      </Card>
...
function Metric({ icon: Icon, label, value }: { icon: React.ComponentType<{ className?: string }>; label: string; value: number }) {
  return <Card><div className="flex items-center gap-2 text-sm text-slate-600"><Icon className="h-4 w-4" />{label}</div><p className="mt-3 text-3xl font-semibold">{value}</p></Card>;
}
